...
class Detector(object):
    ...
    def predict_image(self,
        ...
        with open(self.output_dir+"预测结果.txt","w") as f:
            pass
...
def visualize(image_list, result, labels, output_dir='output/', threshold=0.5):
    ...
    class_name=["Text","Figure","Table","Equation"]
    ...
    for idx, image_file in enumerate(image_list):
        ...
        #--------------------------------------------------------------------------------
        temp_log=im_results['boxes']
        with open(output_dir+"预测结果.txt","a") as f:
            for i in range(len(temp_log)):
                if temp_log[i][1]>threshold:
                    print(img_name,class_name[int(temp_log[i][0])],int(np.round(temp_log[i][2])),int(round(temp_log[i][3])),int(round(temp_log[i][4])),int(round(temp_log[i][5])),file=f)
                else:
                    break
